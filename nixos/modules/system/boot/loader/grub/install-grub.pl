...
if (($requireNewInstall != 0) && ($efiTarget eq "no" || $efiTarget eq "both")) {
    foreach my $dev (@deviceTargets) {
        ...
        my @command = ("$grub/sbin/grub-install", "--recheck", "--modules=nativedisk ahci pata part_gpt part_msdos diskfilter mdraid1x lvm ext2", "--root-directory=$tmpDir", Cwd::abs_path($dev));
