...
if (($requireNewInstall != 0) && ($efiTarget eq "no" || $efiTarget eq "both")) {
    foreach my $dev (@deviceTargets) {
        ...
        my @command = ("$grub/sbin/grub-install", "--recheck", "--modules=pata part_gpt part_msdos diskfilter mdraid1x lvm ext2 nativedisk", "--root-directory=$tmpDir", Cwd::abs_path($dev));
